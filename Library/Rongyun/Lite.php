...
require_once dirname(__FILE__) . '/SDK/ServerAPI.php';
...
class Rongyun_Lite
{
    ...
    public function __construct($appKey = '', $appSecret = '', $format = 'json'){
        $this->appKey = $appKey ? $appKey : DI()->config->get('app.Rongyun.appKey');
        $this->appSecret = $appSecret ? $appSecret : DI()->config->get('app.Rongyun.appSecret');
        $this->format = $format ? $format : DI()->config->get('app.Rongyun.format');
    }
...
//            $paramsString = http_build_query($params);var_dump($paramsString);
     ...
     * @return json|xml
     */
    public function groupJoin($userId, $groupId, $groupName,$requestUserId) {
        try{
            if(empty($userId))
                throw new Exception('被同步群信息的用户 Id 不能为空');
            if(empty($groupId))
            ...
            //XX加入群组的提示
            $user_model=new Model_SNS_Users();
//             print_r($userId);die;
            $requestStr = '';
            foreach ($requestUserId as $k => $v){
            $userInfo=$user_model->getUserInfo($v);
            $requestStr = $requestStr.$userInfo['nickname'].',';
            }
            if($requestStr){
                $requestStr = substr($requestStr, 0, -1);
            }
            $userInfo=$user_model->getUserInfo($userId);
//             $ret1 = $this->curl('/message/group/publish',
//                 array(
//                     'fromUserId' => $userId,
//                     'toGroupId' => $groupId,
//                     'objectName' => 'RC:InfoNtf',
//                     'content' =>'{"message":"'. $userInfo['nickname'].'加入群组","extra":""}',
//                 )
//             );
            if($userId == $requestUserId){
            $context = $userInfo['nickname']."加入本群";
            }else{
            $context = $userInfo['nickname']."邀请".$requestStr."加入本群";
            }
//             print_r($context);die;
            $this->curl('/message/group/publish',
                array(
                    'fromUserId' => $userId,
                    'toGroupId' => $groupId,
                    'objectName' => 'RC:TxtMsg',
                    'content' =>'{"content":"'.$context.'","extra":""}',
                )
            );
            if(empty($ret))
                throw new Exception('请求失败');
     ...
     * @param $content
     */
    public function sendConcernMsg($fromUserId,$toUserId,$objectName,$content)
    {
//         print_r($fromUserId);die;
     ...
     * @return mixed
     */
    public function groupQuit($userId, $groupId,$text = '被移除群组') {
        try{
            if(empty($userId))
                throw new Exception('被同步群信息的用户 Id 不能为空');
            if(empty($groupId))
            ...
            $userInfo=$user_model->getUserInfo($v);
            $str_name = $str_name.$userInfo['nickname'].','; 
            }
           $str_name =  substr($str_name, 0, -1);
            $ret1 = $this->curl('/message/group/publish',
                array(
                    ...
                    'toGroupId' => $groupId,
                    'objectName' => 'RC:InfoNtf',
                    'content' => '{"message":"'.$str_name.$text.'","extra":""}',
                )
            );
//            var_dump($ret1);
            if(empty($ret))
    ...
    public function groupChangeName($userId, $groupId,$name = '新名字') {
        try{
            if(empty($userId))
                throw new Exception('被同步群信息的用户 Id 不能为空');
            if(empty($groupId))
            ...
            $user_model=new Model_SNS_Users();
            $userInfo=$user_model->getUserInfo($userId);
            $ret = $this->curl('/message/group/publish',
                array(
                    'fromUserId' => -1,
                    'toGroupId' => $groupId,
                    'objectName' => 'RC:InfoNtf',
                    'content' => '{"operatorUserId":"'.$userId.'","operation":"Rename","data":"'.$name.'","message":"群主修改本群名为'.$name.'","extra":""}',
                )
            );
            //            var_dump($ret1);
            if(empty($ret))
        ...
        try{
            ...
            $ret1 = $this->curl('/message/group/publish',
                array(
                    ...
                    'toGroupId' => $groupId,
                    'objectName' => 'RC:InfoNtf',
                    'content' => '{"message":"该群已被群组解散","extra":""}',
                )
            );
//            var_dump($ret1);
            return $ret;
        ...
        try{
            ...
            $this->curl('/message/group/publish',
                array(
                    ...
                    'toGroupId' => $groupId,
                    'objectName' => 'RC:TxtMsg',
                    'content' =>'{"content":"大家好，欢迎加入本群","extra":""}',
                )
            );
            if(empty($ret))
                throw new Exception('请求失败');
    ...
    /**
     * 群组发送消息
     * @param unknown $userId 发送用户ID
     * @param unknown $groupId 发送群组ID
     * @param string $type 消息类型
     * @param unknown $context 消息内容
     * @time 2016/9/14
     */
    public function sendGroupNews($userId,$groupId,$type="RC:TxtMsg",$context)
    {
        $this->curl('/message/group/publish',
            array(
                'fromUserId' => $userId,
    ...
    public function demo($fromUserId)
    {
        //         print_r($fromUserId);die;
        $ret = $this->curl('/group/sync',
            array(
                'userId' => 10000226,
                'group[33]' => 'cccc',
            )
