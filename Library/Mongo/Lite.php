 ...
 * Lite.php
 *
 * @author: cnx7 <zysafe@live.cn> 2016-05-11
 */
class Mongo_Lite {

    protected $_configData = array(
        'hostname' => 'mongodb://192.168.200.10:27017/cm_sns',
        'persist' => true,
        'persist_key' => 'mongoqb',
        'replica_set' => false,
        'query_safety' => 'w',
        'dsn' => 'mongodb://192.168.200.10:27017/cm_sns'
    );
    ...
    public function __construct(array $config = [], $connect = true) {
        if (!class_exists('\Mongo')) {
            throw new \Exception('MongoDB 扩展未安装');
        }
...
//    }
    ...
    public function switchDb($dsn = '') {
        if (empty($dsn)) {
            throw new \Exception('如果要切换数据库,必须指定链接');
        }
        ...
        try {
            ...
            throw new \Exception('无法切换至数据库: ' . $Exception->getMessage());
        }
    }
    ...
    public function dropDb($database = '') {
        if (empty($database)) {
            throw new \Exception('删除失败,未指定数据库');
        } else {
            try {
                $this->_connection->{$database}->drop();
                return true;
            } catch (\Exception $Exception) {
                throw new \Exception('删除数据库 `' . $database . '` 失败: ' . $Exception->getMessage());
            }
    ...
    public function dropCollection($database = '', $collection = '') {
        if (empty($database)) {
            throw new \Exception('删除失败,database为空', 500);
        }

        if (empty($collection)) {
            throw new \Exception('删除失败,collection为空', 500);
        } else {
            try {
                $this->_connection->{$database}->{$collection}->drop();
                return true;
            } catch (\Exception $Exception) {
                throw new \Exception('删除collection `' . $collection . '` 失败: ' . $Exception->getMessage(), 500);
    ...
    public function getOne($collection = '') {
        if (empty($collection)) {
            throw new \Exception('请指定Collection');
        }
        ...
        return $cursor->hasNext() ? $cursor->getNext() : array();
    }
    ...
    /**
     * 更新文档
     * @param string $collection 连接名(表名)
    ...
    public function update($collection = '', $options = array()) {
        if (empty($collection)) {
            throw new \Exception('未指定collection');
        }

        if (count($this->updates) === 0) {
            throw new \Exception('未指定更新内容');
        }

        try {
            $options = array_merge(array($this->_querySafety => false, 'multiple' => false), $options);
            ...
            throw new \Exception('更新数据失败: ' . $Exception->getMessage());
        }
    }
