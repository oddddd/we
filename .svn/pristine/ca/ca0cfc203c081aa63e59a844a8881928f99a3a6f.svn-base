...
class PushPayload {
    ...
    const PUSH_VALIDATE_URL = 'https://api.jpush.cn/v3/push';
    private $client;
    private $platform;
    ...
    public function setPlatform($platform) {
        if (is_string($platform) && strcasecmp("all", $platform) === 0) {
            ...
            if (!is_array($platform)) {
                ...
                if (count($platform) <= 0) {
                    self::tuisongError('Missing argument for PushPayload::setPlatform()');
            ...
            foreach($platform as $type) {
                ...
                if (!in_array($type, self::$EFFECTIVE_DEVICE_TYPES)) {
                    self::tuisongError('Invalid device type:');
    ...
    public function setAudience($all) {
        if (strtolower($all) === 'all') {
            ...
            self::tuisongError('Invalid audience value');
    ...
    public function addTag($tag) {
        ...
        if (is_array($tag)) {
            foreach($tag as $_tag) {
                if (!is_string($_tag)) {
                    self::tuisongError('Invalid tag value');
    ...
    public function addTagAnd($tag) {
        ...
        if (is_array($tag)) {
            foreach($tag as $_tag) {
                if (!is_string($_tag)) {
                    self::tuisongError('Invalid tag_and value');
    ...
    public function addIosNotification($alert=null, $sound=null, $badge=null, $content_available=null, $category=null, $extras=null) {
        ...
        if (!is_null($sound)) {
            ...
            if ($sound !== JPush_Lite::DISABLE_SOUND) {
                $ios['sound'] = $sound;
            }
        } else {
            // 默认sound为''
        ...
        if (!is_null($badge)) {
            ...
            if ($badge != JPush_Lite::DISABLE_BADGE) {
                $ios['badge'] = $badge;
            }
        } else {
            // 默认badge为'+1'
    ...
    public function send() {
        $response = $this->client->_request(PushPayload::PUSH_URL, JPush_Lite::HTTP_POST, $this->toJSON());
        return $this->__processResp($response);
    }

    public function validate() {
        $response = $this->client->_request(PushPayload::PUSH_VALIDATE_URL, JPush_Lite::HTTP_POST, $this->toJSON());
        return $this->__processResp($response);
    }
    ...
    private function __processResp($response) {
        if($response['http_code'] === 200) {
            ...
            if (is_array($headers)) {
                ...
                $body['is_success'] = true; //1代表推送成功
                return $body;
            }
            return $body;
        } else {
            self::tuisongError(json_encode($response));
            throw new APIRequestException($response);
        }
    }
    private function generateSendno() {
        //这里出错了 不知道什么原因填写4294967294这个就报错
        return rand(100000, 429496);
    }
    
    private function tuisongError($msg)
    {
        error_log(date('Y-m-d h:i:s').$msg."\r\n",3,dirname(__FILE__).'\tuisong.log');
    }
