...
CREATE TABLE `we_moneyLog` (
 `id` bigint(20) NOT NULL AUTO_INCREMENT,
 `userId` bigint(20) NOT NULL COMMENT '用户id',
 `useType` tinyint(1) NOT NULL COMMENT '使用类型 1用户充值 2用户提现 3用户消费 4管理员操作 5后台每日返利 6提现失败退款 7兑换其他货币',
...
CREATE TABLE `we_chest` (
 ...
 `userId` bigint(20) DEFAULT 0 COMMENT '用户id',
 `day` varchar(10) DEFAULT '' COMMENT '签到日期',
 `time` int(11) DEFAULT '' COMMENT '领取时间戳',
 `money` decimal(15,2) DEFAULT 0.00 comment '领取的金额',
 `walletId` tinyint(1) DEFAULT '1' COMMENT '钱包类型 1钱包 2金币钱包',
 `createTime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`)
...
CREATE TABLE `we_task` (
 ...
 `money` decimal(15,2) DEFAULT 0.00 comment '领取的金额',
 `walletId` tinyint(1) DEFAULT '1' COMMENT '钱包类型 1钱包 2金币钱包',
 `taskType` tinyint(1) DEFAULT 0 comment '任务类型 1输入邀请码 2新手阅读 3分享晒一晒到朋友圈 4分享文章 5阅读咨询 6优质评论 7阅读推荐咨询 8调查问卷',
 `createTime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`)
...
CREATE TABLE `we_share` (
 ...
 `openid` INT(11) DEFAULT 0 COMMENT '被打开过多少次',
