...
$api = isset($_POST['serv'])?$_POST['serv']:'';

$user_center = new medoo([
    'database_type' => 'mysql',
    'database_name' => 'CM_UserCenter',
...
switch($api){
    ...
    case "deluser":
        ...
        if(empty($_POST['tel'])){
            echo '<script>window.location = "/v2/testApi.php";</script>';
        }

        $uid = $user_center->get('member', 'id', ['tel' => $_POST['tel']]);

        $sns->delete('concern', ['users_id' => $uid]);
        $sns->delete('concern', ['concern_id' => $uid]);

        $sns->delete('microblog_like', ['users_id' => $uid]);
        $sns->delete('microblog_comment', ['users_id' => $uid]);
        $sns->delete('microblog', ['users_id' => $uid]);

        $sns->delete('pets', ['users_id' => $uid]);
        $sns->delete('users', ['id' => $uid]);

        $user_center->delete('user_login_qq', ['member_id' => $uid]);
        $user_center->delete('user_login_sina', ['member_id' => $uid]);
        $user_center->delete('user_login_weixin', ['member_id' => $uid]);
        $user_center->delete('member', ['id' => $uid]);

        $user_center->delete('session', ['member_id' => $uid]);

        echo '<script>alert("删除成功");window.location = "/v2/testApi.php";</script>';
    ...
    case "lockuser":
        ...
        $user_center->update('member', ['islock' => $_POST['lock']], ['tel' => $_POST['tel']]);
        $user_center->update('session', ['islock' => $_POST['lock']], ['member_id' => $uid]);

        echo '<script>alert("操作成功");window.location = "/v2/testApi.php";</script>';
    ...
    default:
        ...
        echo '<script>alert("没有提交的内容哦");window.location = "/v2/testApi.php";</script>';
