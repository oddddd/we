 ...
 */
class Model_WeMoneyLog extends PhalApi_Model_NotORM
{
    /**
    ...
    {
        try {
            $sql = "select * from we_moneyLog {$sql}";
    ...
    {
        try {
            $sql = "select count(*) as page from we_moneyLog {$sql}";
            $list = $this->getORM()->queryAll($sql);
    ...
    {
        try {
            $sql = "delete from we_moneyLog where id = {$data['id']}";
