 ...
 */
class Model_WeCommentTask extends PhalApi_Model_NotORM
{
    /**
    ...
    {
        try {
            $sql = "select * from we_commentTask {$sql}";
     ...
     * @throws PhalApi_Exception_InternalServerError
     */
    public function getOne($fid,$type)
    {
        try {
            $sql = "select * from we_commentTask where `fid` = {$fid} && `type` = '{$type}'";
            $list = $this->getORM()->queryAll($sql);
     ...
     * @throws PhalApi_Exception_InternalServerError
     */
    public function getListByType($userId,$day)
    {
        try {
            $sql = "select * from we_commentTask where `userId` = {$userId} && `day` = '{$day}'";
            $list = $this->getORM()->queryAll($sql);
            return $list;
        } catch (Exception $ex) {
            DI()->logger->error('SQL_ERROR', ['trace' => __METHOD__, 'msg' => $ex->getMessage()]);
    ...
    {
        try {
            $sql = "select count(*) as page from we_commentTask {$sql}";
            $list = $this->getORM()->queryAll($sql);
            return isset($list[0])?$list[0]:null;
        } catch (Exception $ex) {
            DI()->logger->error('SQL_ERROR', ['trace' => __METHOD__, 'msg' => $ex->getMessage()]);
    ...
    {
        try {
            $sql = "select * from we_commentTask";
            $list = $this->getORM()->queryAll($sql);
            return $list;
        } catch (Exception $ex) {
            DI()->logger->error('SQL_ERROR', ['trace' => __METHOD__, 'msg' => $ex->getMessage()]);
    ...
    {
        try {
            $sql = "delete from we_commentTask where id = {$data['id']}";
