 ...
 * WeExchangeLog.php
 *
 * @author: wjp 2018-02-22
 */
class Model_WeExchangeLog extends PhalApi_Model_NotORM
{
    /**
    ...
    {
        try {
            $sql = "select * from we_exchangeLog {$sql}";
    ...
    {
        try {
            $sql = "select count(*) as page from we_exchangeLog {$sql}";
            $list = $this->getORM()->queryAll($sql);
    ...
    {
        try {
            $sql = "select * from we_exchangeLog";
            $list = $this->getORM()->queryAll($sql);
            return $list;
        } catch (Exception $ex) {
            DI()->logger->error('SQL_ERROR', ['trace' => __METHOD__, 'msg' => $ex->getMessage()]);
     ...
     * @throws PhalApi_Exception_InternalServerError
     */
    public function getListByDay($day)
    {
        try {
            $sql = "select * from we_exchangeLog where day = '{$day}'";
            $list = $this->getORM()->queryAll($sql);
            return isset($list[0])?$list[0]:null;
        } catch (Exception $ex) {
            DI()->logger->error('SQL_ERROR', ['trace' => __METHOD__, 'msg' => $ex->getMessage()]);
    ...
    {
        try {
            $sql = "delete from we_exchangeLog where id = {$data['id']}";
