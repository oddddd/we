 ...
 */
class Model_WeSignIn extends PhalApi_Model_NotORM
{
    /**
    ...
    {
        try {
            $sql = "select count(*) as page from we_signIn {$sql}";
            $list = $this->getORM()->queryAll($sql);
     ...
     * @throws PhalApi_Exception_InternalServerError
     */
    public function getInfoByUserId($userId,$date)
    {
        try {
            $sql = "select * from we_signIn where `day` = '{$date}' && `userId` = {$userId}";
            $list = $this->getORM()->queryAll($sql);
            return isset($list[0])?$list[0]:null;
        } catch (Exception $ex) {
            DI()->logger->error('SQL_ERROR', ['trace' => __METHOD__, 'msg' => $ex->getMessage()]);
    ...
    {
        try {
            $sql = "delete from we_signIn where id = {$data['id']}";
