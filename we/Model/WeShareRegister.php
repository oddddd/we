 ...
 * WeShareRegister.php
 * 分享到朋友圈注册页面
 * @author: wjp 2018-02-28
 */
class Model_WeShareRegister extends PhalApi_Model_NotORM
{
    /**
    ...
    {
        try {
            $sql = "select * from we_shareRegister {$sql}";
    ...
    {
        try {
            $sql = "select count(*) as page from we_shareRegister {$sql}";
            $list = $this->getORM()->queryAll($sql);
    ...
    {
        try {
            $sql = "select * from we_shareRegister";
            $list = $this->getORM()->queryAll($sql);
            return $list;
        } catch (Exception $ex) {
            DI()->logger->error('SQL_ERROR', ['trace' => __METHOD__, 'msg' => $ex->getMessage()]);
     ...
     * @throws PhalApi_Exception_InternalServerError
     */
    public function getListByIdDay($userId,$day)
    {
        try {
            $sql = "select * from we_chest where `userId` = {$userId} && `day` = {$day} ORDER BY getTime desc";
            $list = $this->getORM()->queryAll($sql);
            return isset($list[0])?$list[0]:null;
        } catch (Exception $ex) {
            DI()->logger->error('SQL_ERROR', ['trace' => __METHOD__, 'msg' => $ex->getMessage()]);
    ...
    {
        try {
            $sql = "delete from we_shareRegister where id = {$data['id']}";
