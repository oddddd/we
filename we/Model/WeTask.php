 ...
 */
class Model_WeTask extends PhalApi_Model_NotORM
{
    /**
     ...
     * @throws PhalApi_Exception_InternalServerError
     */
    public function getOne($userId,$day,$taskType)
    {
        try {
            $sql = "select * from we_task where `userId` = {$userId} && `day` = '{$day}' && taskType = '{$taskType}'";
            $list = $this->getORM()->queryAll($sql);
     ...
     * @throws PhalApi_Exception_InternalServerError
     */
    public function getListByType($userId,$day,$taskType)
    {
        try {
            $sql = "select * from we_task where `userId` = {$userId} && `day` = '{$day}' && taskType = '{$taskType}'";
            $list = $this->getORM()->queryAll($sql);
    ...
    {
        try {
            $sql = "select count(*) as page from we_task {$sql}";
            $list = $this->getORM()->queryAll($sql);
            return isset($list[0])?$list[0]:null;
        } catch (Exception $ex) {
            DI()->logger->error('SQL_ERROR', ['trace' => __METHOD__, 'msg' => $ex->getMessage()]);
