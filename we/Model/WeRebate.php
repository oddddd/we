 ...
 */
class Model_WeRebate extends PhalApi_Model_NotORM
{
    /**
    ...
    {
        try {
            $sql = "select count(*) as page from we_rebate {$sql}";
            $list = $this->getORM()->queryAll($sql);
     ...
     * @throws PhalApi_Exception_InternalServerError
     */
    public function getInfoByUserId($userId,$sonId)
    {
        try {
            $sql = "select * from we_rebate where userId = {$userId} && sonId = {$sonId} ";
            $list = $this->getORM()->queryAll($sql);
            return $list;
        } catch (Exception $ex) {
            DI()->logger->error('SQL_ERROR', ['trace' => __METHOD__, 'msg' => $ex->getMessage()]);
    ...
    {
        try {
            $sql = "delete from we_rebate where id = {$data['id']}";
